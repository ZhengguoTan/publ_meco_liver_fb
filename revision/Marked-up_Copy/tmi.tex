...
model-based reconstruction. However, this work did not allow \hl{update $B_0$ maps
and coil sensitivities during motion-resolved model-based reconstruction, which
is problematic because field fluctuations induced by respiratory motion
result in temporarily varying $B_0$ maps and coil sensitivities. Here,
we integrate joint $B_0$ estimation into the reconstruction, which is
challenging because the $B_0$ field map term appears as a non-linear
term in the water/fat separation problem and it may change rapidely
at water-tissue interfaces.} \marginnote{R1.7}
\hl{Joint estimation of the field map was proposed before using
a smoothness prior on the $B_0$ field map  via first- or second-order
finite-difference regularization 
\mbox{\cite{sutton_2004_dynamicfield,olafsson_2008_joint,funai_2008_secondorder}},
which is insufficient in case of such more pronounced field inhomogeneities}. \marginnote{R1.8}
...
can result in the cartoon-like artifacts, whereas temporal TV regularization is
beneficial to remove  streaking artifacts.
}%FIXME: check schneider
