...
\marginnote{R1.1}
\hl{Moreover, it is advantageous to update $B_0$ during model-based 
iterative reconstruction. 
As shown in \mbox{\cref{FIG:B0}}, the current initialization strategy used 
...
\begin{figure*}
	...
	\caption{(A) Initialized $B_0$ maps of four respiratory bins 
		...
		Joint update using seven echoes supplies more details in $B_0$ maps.}
	\label{FIG:B0}
\end{figure*}
...
\begin{figure*}
	\centering
	\includegraphics[width=\textwidth]{../../figures/tan13.pdf}
	\caption{Magnitude of coil sensitivity maps on Patient \#1 from 
		(top) the proposed model-based method and (bottom) the ESPIRiT method.}
	\label{FIG:SENS}
\end{figure*}

\marginnote{R1.2}
\hl{This work focuses on joint estimation of all parameter maps 
via solving a nonlinear inverse problem, 
whereas other reconstruction techniques calibrate coil sensitivity maps 
before image reconstruction. 
The state-of-the-art coil sensitivity calibration method is 
ESPIRiT \mbox{\cite{uecker_2014_espirit}}. 
Therefore, \mbox{\cref{FIG:SENS}} displays the estimated coil sensitivity maps 
from the joint estimation method and ESPIRiT, respectively. 
The spatial brightness of coil sensitivity maps from both methods is similar, 
but the joint estimation method does not show bright signal in regions outside the subject.
ESPIRiT supplies a thresholding parameter for sensitivity cropping, 
which was not used here for fair comparison.
}
